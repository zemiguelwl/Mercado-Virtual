...
async function register(req, res, next) {
  try {
    ...
    if (existingUser && !existingUser.isEmailVerified) {
      ...
      // isto impede que um registo via POS converta-se num cliente POS num supermercado ou estafeta
...
async function showVerifyEmail(req, res, next) {
  try {
...
async function verifyEmail(req, res, next) {
  try {
    ...
    if (user) {
      try {
        // Envia cupao de boas-vindas apenas após a verificação de email
...
async function processLogin(req, res, next) {
  try {
    ...
    // 5. Password correcta? só aqui é que é seguro chamar comparePassword
